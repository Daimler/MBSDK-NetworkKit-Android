...
/**
 * Creates an [HttpLoggingInterceptor] that uses [MBLoggerKit] to log request and response outputs
...
fun <T : RequestError> defaultErrorMapping(error: Throwable?, errorClass: Class<T>?): ResponseError<out RequestError> {
    return when (error) {
        ...
        is BaseRetrofitTask.ResponseException -> {
