...
abstract class ChaineableMessageProcessor(private val nextProcessor: MessageProcessor? = null) : MessageProcessor {
     ...
     */
    protected abstract fun doHandleReceivedMessage(notifyable: Notifyable, sendService: SendMessageService, dataSocketMessage: DataSocketMessage): Boolean

    /**
     * This is call if there is a message that should be parsed to send. If null was returned, the
     * message will be passed to the next configured [MessageProcessor]. Else, the processing will stop
     * and the result is returned
     */
    protected abstract fun doHandleMessageToSend(message: SendableMessage): DataSocketMessage?
}