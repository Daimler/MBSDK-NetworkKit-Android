...
abstract class BaseTask<C, F> : FutureTask<C, F> {
    ...
    private var allwaysCallback: ((TaskState, C?, F?) -> Unit)? = null
    ...
    protected fun triggerAlways(state: TaskState, completeResult: C?, failureResult: F?) {
        allwaysCallback?.invoke(state, completeResult, failureResult)
        allwaysCallback = null
    }
    ...
    override fun onAlways(callback: (TaskState, C?, F?) -> Unit): FutureTask<C, F> {
        synchronized(this) {
            if (isPending()) {
                allwaysCallback = callback
            } else {
                triggerAlways(state, completeResult, failureResult)
            }
        }
        return this
    }
