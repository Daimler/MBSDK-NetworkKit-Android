...
open class TaskObject<C, F> : BaseTask<C, F>(), Task<C, F> {
    ...
    final override fun complete(result: C) {
        synchronized(this) {
            if (isPending().not()) {
                throw BaseTask.AlreadyFinishedException("TaskObject was already finished, cannot complete again")
            }
    ...
    final override fun fail(result: F) {
        synchronized(this) {
            if (isPending().not()) {
                throw BaseTask.AlreadyFinishedException("TaskObject was already finished, cannot faile again")
            }
