...
class ReconnectSocketStateTest {
    ...
    @Test
    fun reconnectNotTriggeredIfFailedBecauseForbidden() {
        val delay = 500L
        val retries = 1
        val reconnect = FixPeriodicReconnection(delay, retries)
    ...
    inner class TestReconnactableSocketConnection(reconnection: Reconnection) : ReconnectableSocketConnection(reconnection, TestMessageProcessor()) {
    ...
    inner class FailingSocketConnection(private val connectionError: ConnectionError, reconnection: Reconnection) : ReconnectableSocketConnection(reconnection, TestMessageProcessor()) {
