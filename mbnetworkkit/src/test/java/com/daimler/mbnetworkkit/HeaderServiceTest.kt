...
class HeaderServiceTest {
    ...
    @Test
    fun testCustomLocaleHeader() {
        val interceptor = MBNetworkKit.headerService().createRisHeaderInterceptor()

        val customHeaders = mapOf(HEADER_LOCALE to "custom_locale")
        val chain = CustomHeadersChain(customHeaders)
        interceptor.intercept(chain)
        val request = chain.request()
        assertEquals("custom_locale", request.header(HEADER_LOCALE))
    }
